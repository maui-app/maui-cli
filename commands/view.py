...
def view_expenses(date: datetime):
    try:
        ...
        response = client.execute(DAILYEXPENSES, variable_values=variables)["dailyExpenses"]
...
def parse_expenses_text(data, date: datetime):
    ...
    day = "today" if str(date).split(' ')[0] == str(dt.today()) else "on {0}".format(get_formatted_date_string(date))
